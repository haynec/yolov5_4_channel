...
#            img0 = cv2.imread(path)  # BGR
            img0 = cv2.imread(path, cv2.IMREAD_UNCHANGED)  # for reading 4-channels png
            # end danny_change
...
#            im = cv2.imread(str(im_file))[..., ::-1]  # BGR to RGB
            im = cv2.imread(str(im_file), cv2.IMREAD_UNCHANGED) # BGR to RGB
            im[:,:,[0,1,2]] = im[:,:,[2,1,0]]  # swaps RGB channels while preserving fourth channel (assumes first three channels are RGB)
            # end danny_change
            h, w = im.shape[:2]
